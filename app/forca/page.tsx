    ...
    <main className="flex flex-col items-center justify-center min-h-screen bg-[#FDF5E6] text-[#4A3728] p-4">
      ...
      <div className="bg-[#FAF0E6] border-2 border-[#D2B48C] p-6 md:p-10 rounded-[40px] shadow-xl flex flex-col items-center max-w-2xl w-full">
        ...
        <header className="flex flex-col items-center mb-6">
          <h1 className="text-4xl font-serif font-bold text-[#5D4037]">
            Jogo da Forca
          </h1>
        ...
        <section className="flex flex-col items-center w-full mb-8">
          ...
          <span className="text-sm font-bold text-[#8B4513] uppercase tracking-[0.3em] mb-6">
            Adivinhe o Time
          </span>
          ...
          <div className="flex flex-wrap justify-center gap-1 sm:gap-2 md:gap-4 mb-10 max-w-full">
            {palavra.split("").map((letra, i) => (
              <div 
                key={i} 
                className={`w-6 h-10 sm:w-8 sm:h-12 flex items-center justify-center ${
                  letra === " " ? "border-b-0 w-3" : "border-b-4 border-[#3E2723]"
                }`}
              >
                <span className="text-xl sm:text-2xl md:text-3xl font-mono font-bold">
          ...
          <div className="bg-[#5D4037] px-6 py-2 sm:px-8 sm:py-3 rounded-full shadow-lg">
            <span className="text-[#FAF0E6] text-xs sm:text-sm font-bold uppercase tracking-widest">
              ...
              {linha.split("").map((letra) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-10 sm:w-10 sm:h-12 md:w-12 md:h-14 rounded-xl font-bold transition-all duration-150 shadow-md border-b-4 
                      ${!jaUsou 
                        ? "bg-[#D2B48C] border-[#A0522D] text-[#3E2723] active:translate-y-1 active:border-b-0" 
                        : correta 
                          ? "bg-[#6B8E23] border-[#556B2F] text-white" 
                          : "bg-[#BC8F8F]/40 border-transparent text-[#8B4513]/50 opacity-60"}
                    `}
                  >
                    {letra}
                  </button>
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          {(venceu || perdeu) && (
            <div className={`p-4 rounded-2xl font-bold text-center w-full ${
          ...
          <button
            onClick={iniciarJogo}
            className="bg-[#5D4037] text-white px-10 py-3 rounded-full font-bold uppercase shadow-xl w-full md:w-auto"
          >
            {venceu || perdeu ? "Próxima Partida" : "Reiniciar Jogo"}
          </button>

          <Link href="/" className="w-full md:w-auto text-center">
            <button className="border-2 border-[#5D4037] text-[#5D4037] px-10 py-3 rounded-full font-bold w-full">
              Voltar ao Início
            </button>
          </Link>
        </div>
