...
const PALAVRAS = [
  "SPORT", "FLAMENGO", "PALMEIRAS", "CORINTHIANS", "SAOPAULO", "SANTOS",
  "GREMIO", "INTERNACIONAL", "CRUZEIRO", "ATLETICO", "BAHIA", "VITORIA",
  "CEARA", "FORTALEZA", "NAUTICO", "SANTACRUZ", "BOTAFOGO", "VASCO",
  "FLUMINENSE", "JUVENTUDE", "GOIAS", "CORITIBA", "AMERICA", "CHAPECOENSE",
  "BRAGANTINO", "MIRASSOL", "PONTEPRETA", "GUARANI", "CSA", "ABC"
];
...
export default function Forca() {
  ...
  useEffect(() => { iniciarJogo(); }, [iniciarJogo]);

  const venceu = palavra !== "" && palavra.split("").every((l) => letrasUsadas.includes(l));
    ...
    <main className="flex flex-col items-center justify-center min-h-screen bg-[#FDF5E6] text-[#4A3728] p-4">
      ...
      <div className="bg-[#FAF0E6] border-2 border-[#D2B48C] p-6 md:p-10 rounded-[40px] shadow-xl flex flex-col items-center max-w-2xl w-full">
        ...
        <section className="flex flex-col items-center w-full mb-8">
          ...
          {/* CAMPOS DAS LETRAS - SEMPRE COM O TRAÇO EMBAIXO */}
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-10">
            {palavra.split("").map((letra, i) => (
              <div 
                key={i} 
                className="w-8 h-12 border-b-4 border-[#3E2723] flex items-center justify-center"
              >
        ...
        <div className="flex flex-col gap-3 w-full items-center mb-8">
          {["QWERTYUIOP", "ASDFGHJKL", "ZXCVBNM"].map((linha, idx) => (
            <div key={idx} className="flex gap-2 w-full justify-center">
              {linha.split("").map((letra) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 max-w-[45px] h-12 md:h-14 rounded-xl font-bold transition-all duration-150 shadow-md border-b-4 
                      ${!jaUsou 
        ...
        {/* Mensagens e Botão Próxima Partida */}
        <div className="flex flex-col items-center gap-4 w-full">
          {(venceu || perdeu) && (
            <div className={`p-4 rounded-2xl font-bold text-center w-full animate-in fade-in zoom-in duration-300 ${
              ...
              {venceu ? "✨ Parabéns! O time era " + palavra : `⌛ Não foi dessa vez. Era: ${palavra}`}
            </div>
          )}
          
          <button
            onClick={iniciarJogo}
            className="bg-[#5D4037] hover:bg-[#3E2723] text-white px-12 py-4 rounded-full font-bold uppercase tracking-widest shadow-xl transition-all active:scale-95"
          >
